...
* Date last modified: 24 April 2017
* CSCE350 Program4_Assignment (Warshalls and Floyds)
*
*
*
**/
	...
	public:
		...
		********************************************/
		std::vector<std::vector<bool> > getTransitiveClosure(){
			...
			//R(0) = A
			std::vector<std::vector<bool> > R_Old = R;
			std::vector<std::vector<bool> > R_New = R;
			...
			for (int k = 0; k < R.size(); k++)
			{
			  ...
			  for (int i = 0; i < R.size(); i++)
			  {
				...
				R_New[i][j] = R_Old[i][j] || (R_Old[i][k] && R_Old[k][j]);		      
			    }
			  }
			}
			R = R_New;
		...
		std::vector<std::vector<W> > getAllPairsShortestPaths(){
			...
			for (int k = 0; k < D.size(); k++)
			{
			  for (int i = 0; i < D.size(); i++)
			  {
			    ...
			    { 
			      // First assign all diagonal elements to zero, then follow alg.
			      D[k][k] = 0;
